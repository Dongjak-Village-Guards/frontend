...
const CardText = ({ store }) => {
  ...
  const featuredMenu = (() => {
    ...
    if (store.hasDesigners && store.designers?.length > 0) {
      ...
      if (allMenus.length === 0) {
        return { discount_rate: 0, menu_price: 0, discounted_price: 0, menu_name: '메뉴 없음' };
      }
      return allMenus.reduce(
        (prev, curr) => (prev.discount_rate > curr.discount_rate ? prev : curr),
        { discount_rate: 0, menu_price: 0, discounted_price: 0, menu_name: '메뉴 없음' }
      );
    }
    // 디자이너가 없는 경우
  ...
  return (
    <StoreInfo>
      <InfoWrapperLeft>
        {/* 제목 6글자까지 표시 */}
        <StoreName>{name?.length > 6 ? `${name.slice(0, 6)}...` : name || '가게명'}</StoreName>
        <StoreMeta>
      ...
      <InfoWrapperRight>
        ...
        <StorePrice>
            {featuredMenu.discount_rate}%
            <span>{featuredMenu.menu_price.toLocaleString()}원</span> {/* CSS 분리할려고 span에 감쌈 */}
        </StorePrice>
        <StoreDiscountPrice>
            {featuredMenu.discounted_price.toLocaleString()}원
        </StoreDiscountPrice>
      </InfoWrapperRight>
    </StoreInfo>
  );
