 ...
 * @param {string} store.store_id - 가게 ID
 * @param {string} store.store_name - 가게 이름 (alt 텍스트용)
 */
const StoreCard = ({ store }) => {
  ...
  const imageMap = {
    ...
    7: hairImage,
      ...
      const imageSrc = imageMap[store.store_id] || placeholderImage;
  ...
  return (
    <CardImageContainer>
      <ImageGroup>
        <MainCardImage
          src={imageSrc}
                      alt={`${store.store_name} 메인 이미지`}
          onError={(e) => {
            console.warn(`이미지 로드 실패: ${store.store_id}, using fallback`);
